...
=======
    ...
    def doAction(self,data):
        ...
        user = helpers.evalString(self.user,{"data" : data["flowData"]})
        port = helpers.evalString(self.port_forward,{"data" : data["flowData"]})
>>>>>>> f0703476d5833c1dcfec0d6aed35cd7761288c2b
        ...
        if self.isPortForward:
            if keyfile != "":
                if password:
                    ...
                    client = linux.linux(
                        host, user, keyFile=keyfile, port_forward=True, port=port
                    )
            else:
...
>>>>>>> f0703476d5833c1dcfec0d6aed35cd7761288c2b
            ...
            return True
        return super(_remoteConnectLinux, self).setAttribute(
...
>>>>>>> f0703476d5833c1dcfec0d6aed35cd7761288c2b
            self.password = "ENC {0}".format(auth.getENCFromPassword(value))
            return True
        return super(_remoteConnectWindows, self).setAttribute(
            attr, value, sessionData=sessionData
        )
...
class _remoteConnectFortigate(action._action):
    ...
    def doAction(self, data):
        ...
        deviceHostname = helpers.evalString(
            self.deviceHostname, {"data": data["flowData"]}
        )
        user = helpers.evalString(self.user, {"data": data["flowData"]})
        if self.password.startswith("ENC"):
            password = auth.getPasswordFromENC(self.password)
...
>>>>>>> f0703476d5833c1dcfec0d6aed35cd7761288c2b
            self.password = "ENC {0}".format(auth.getENCFromPassword(value))
            return True
        return super(_remoteConnectFortigate, self).setAttribute(
            attr, value, sessionData=sessionData
        )
...
class _remoteCommand(action._action):
    ...
    def doAction(self, data):
        ...
        try:
            ...
            client = None
        if client:
            exitCode, output, errors = client.command(
                command, elevate=self.elevate, runAs=self.runAs, timeout=self.timeout
            )
...
class _remoteDownloadConfig(action._action):
    ...
    def doAction(self, data):
        ...
        data["eventData"]["backup_args"] = {"dst_folder": self.dstFolder}
...
class _remoteUpload(action._action):
    ...
    def doAction(self, data):
        ...
        if self.useStorage:
            try:
                localFilePath = (
                    jimi.storage._storage()
                    .getAsClass(id=localFile)[0]
                    .getLocalFilePath()
                )
                ...
                return {
                    "result": False,
                    "rc": 404,
                    "msg": "Local file not found within storage store. storageID={0}".format(
                        localFile
                    ),
                }
